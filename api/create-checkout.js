...
const PLANS = {
  autonomous_fleet: { name: 'Autonomous Fleet AI', price_cents: 39900, trial_days: 14, full_price_cents: 59900 },
}
...
export default async function handler(req) {
  ...
  try {
    ...
    // $399/truck/mo (founder pricing), $599/truck after first 50
    const trucks = Math.max(1, parseInt(truckCount) || 1)
    const perTruckCents = plan.price_cents  // 39900 = $399
    const totalCents = perTruckCents * trucks
    ...
    params.append('line_items[0][price_data][product_data][name]', `Qivori AI — ${plan.name} (${trucks} truck${trucks > 1 ? 's' : ''})`)
    ...
    params.append('subscription_data[metadata][per_truck_cents]', perTruckCents.toString())
