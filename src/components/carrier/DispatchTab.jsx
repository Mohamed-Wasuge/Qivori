        ...
        <div style={{ background: 'var(--surface)', border: '1px solid rgba(240,165,0,0.3)', borderRadius: 12, padding: 20 }}>
          ...
          {form.rate && (
            <div style={{ marginBottom: 12, padding: '10px 14px', background: 'rgba(240,165,0,0.06)', border: '1px solid rgba(240,165,0,0.2)', borderRadius: 8, fontSize: 12, display: 'flex', gap: 24, flexWrap: 'wrap', alignItems: 'baseline' }}>
              ...
              {form.miles && <span>Est. Net: <b style={{ color: 'var(--success)' }}>${(() => { const _g = parseFloat(form.rate||0), _m = parseFloat(form.miles||0), _d = (ctxDrivers||[]).find(d => (d.name||d.full_name||d.driver_name) === form.driver), _dp = _d?.pay_rate ? (_d.pay_model==='percent' ? _g*(Number(_d.pay_rate)/100) : _d.pay_model==='permile' ? _m*Number(_d.pay_rate) : _d.pay_model==='flat' ? Number(_d.pay_rate) : _g*(Number(_d.pay_rate)/100)) : _g*0; return Math.round(_g - _m/6.8*3.89 - _dp).toLocaleString() })()}</b></span>}
            </div>
          )}
