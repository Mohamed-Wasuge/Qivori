...
export function LoadTrackingPage({ token }) {
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase}/api/load-tracking?token=${encodeURIComponent(token)}`)
      .then(r => r.json())
      .then(d => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0e', fontFamily: "'DM Sans',-apple-system,BlinkMacSystemFont,sans-serif" }}>
      ...
      <div style={{ borderBottom: '1px solid #1e1e2a', padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <span style={{ fontSize: 11, color: '#8a8a9a', marginLeft: 16, letterSpacing: 1 }}>LOAD TRACKING</span>
      </div>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '40px 20px' }}>
        ...
        {error && (
          <div style={{ textAlign: 'center', padding: 60 }}>
            ...
            <div style={{ color: '#ef4444', fontSize: 16, fontWeight: 700, marginBottom: 8 }}>Load Not Found</div>
            <div style={{ color: '#8a8a9a', fontSize: 13 }}>This tracking link may be expired or invalid.</div>
          </div>
        )}
          ...
          <>
            ...
            <div style={{ background: '#16161e', border: '1px solid #2a2a35', borderRadius: 16, padding: 24, marginBottom: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                ...
                <div style={{ padding: '6px 16px', borderRadius: 8, background: (STATUS_COLORS[data.status] || '#f0a500') + '18', border: '1px solid ' + (STATUS_COLORS[data.status] || '#f0a500') + '40' }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: STATUS_COLORS[data.status] || '#f0a500' }}>{data.status}</span>
                </div>
              </div>
                ...
                {(data.timeline || []).map((step, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{
                      ...
                      background: step.completed ? '#f0a500' : '#2a2a35',
                ...
                {data.stops.map((stop, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0', borderBottom: i < data.stops.length - 1 ? '1px solid #2a2a35' : 'none' }}>
                    <div style={{
                      ...
                      background: stop.departed ? '#22c55e' : stop.arrived ? '#f0a500' : '#2a2a35',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      fontSize: 9, color: stop.departed || stop.arrived ? '#000' : '#555', fontWeight: 700,
                    }}>{i + 1}</div>
                    <div style={{ flex: 1 }}>
                    ...
                    {stop.date && <span style={{ fontSize: 11, color: '#8a8a9a' }}>{new Date(stop.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>}
                  </div>
                ))}
              </div>
            )}
