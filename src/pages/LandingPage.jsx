...
export default function LandingPage() {
  ...
  useEffect(() => {
    fetch('/api/pricing').then(r => r.json()).then(d => setFounderSpots(d.founderSpotsRemaining ?? 100)).catch(() => setFounderSpots(100))
  }, [])
    ...
    <div className='landing-wrap'>
      ...
      <header className='landing-nav'>
        ...
        <div className='landing-nav-actions'>
          <button className='btn btn-ghost' onClick={() => navigate('/login?mode=signin')}>Sign In</button>
          <button className='btn btn-primary' onClick={() => navigate('/login?mode=signup')}>Start Free Trial</button>
        </div>
      </header>
      <main className='landing-main'>
        <section className='landing-hero'>
          ...
          <div className='landing-hero-actions'>
            <button className='btn btn-primary' style={{padding:'12px 32px',fontSize:14}} onClick={() => navigate('/login?mode=signup')}>Start 14-Day Free Trial</button>
        ...
        <section style={{maxWidth:520,margin:'0 auto 60px',padding:'0 24px'}} id='pricing'>
          ...
          <div className='panel' style={{padding:32,border:'1px solid var(--accent)',borderRadius:16,position:'relative',overflow:'hidden'}}>
            ...
            <button className='btn btn-primary' style={{width:'100%',marginTop:16,padding:'14px 0',fontSize:15}} onClick={() => navigate('/login?mode=signup')}>Start Free Trial \u2014 $0 for 14 Days</button>
          </div>
        </section>
        ...
        <section style={{textAlign:'center',padding:'40px 24px 60px'}}>
          ...
          <button className='btn btn-primary' style={{padding:'12px 32px'}} onClick={() => navigate('/login?mode=signup')}>Start Free Trial</button>
        </section>
      </main>
