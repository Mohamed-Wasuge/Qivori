...
import {
  ...
  Siren, Award, Truck, Star, TrendingUp, Package, BarChart2, UserPlus
} from 'lucide-react'
...
export function PayrollTracker() {
  ...
  const downloadSettlementPDF = (driverData) => {
    const { start, end } = getDateRange()
    const d = driverData
    const lines = [
      ...
      `Pay Model: ${d.driver.pay_model === 'percent' ? `${d.driver.pay_rate||28}%` : d.driver.pay_model === 'permile' ? `$${d.driver.pay_rate}/mi` : `$${d.driver.pay_rate} flat`}`,
      ``,
      `LOADS (${d.loads.length})`,
      `${'Load ID'.padEnd(15)} ${'Route'.padEnd(30)} ${'Miles'.padEnd(10)} ${'Gross'.padEnd(12)}`,
      `${'-'.repeat(67)}`,
    ]
    d.loads.forEach(l => {
      lines.push(`${(l.load_number||l.id||'—').toString().padEnd(15)} ${`${l.origin||'?'} → ${l.destination||'?'}`.padEnd(30)} ${String(l.miles||0).padEnd(10)} $${Number(l.rate||0).toLocaleString()}`)
    })
    lines.push(``, `EARNINGS`, `  Gross Revenue: $${d.totalGross.toLocaleString(undefined,{minimumFractionDigits:2})}`, `  Driver Pay: $${d.driverPay.toLocaleString(undefined,{minimumFractionDigits:2})}`)
    if (d.deductions > 0) lines.push(``, `DEDUCTIONS: -$${d.deductions.toLocaleString(undefined,{minimumFractionDigits:2})}`)
    d.recurringDeductions.forEach(r => lines.push(`  ${r.label}: -$${Number(r.amount).toFixed(2)}`))
    d.manualDeductions.forEach(r => lines.push(`  ${r.label}: -$${Number(r.amount).toFixed(2)}`))
    lines.push(``, `${'='.repeat(40)}`, `NET PAY: $${d.netPay.toLocaleString(undefined,{minimumFractionDigits:2})}`, ``, `Generated by Qivori AI · ${new Date().toLocaleString()}`)
    const blob = new Blob([lines.join('\n')], { type: 'text/plain' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href = url; a.download = `settlement-${d.name.replace(/\s/g,'-')}-${start.toISOString().slice(0,10)}.txt`; a.click()
    URL.revokeObjectURL(url)
    showToast('','Downloaded',`Settlement for ${d.name}`)
  }
              ...
              {runPayrollData.map(d => (
                <div key={d.driver.id} style={{ ...ps.panel, padding:'20px 24px' }}>
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
                    ...
                    <button onClick={() => downloadSettlementPDF(d)} className="btn btn-ghost" style={{ fontSize:11, padding:'5px 10px' }}>
